...
internal enum APISimulator {
	...
	public static func simulateFailure(endPoint: EndpointType, error: Error) -> HTTPStubsDescriptor {
		stub { request in
	...
	public static func stubAPI(endPoint: EndpointType, statusCode: StatusCode, response: Response) -> HTTPStubsDescriptor {
		stub { request in
			request.url?.path == endPoint.path
		} response: { request in
