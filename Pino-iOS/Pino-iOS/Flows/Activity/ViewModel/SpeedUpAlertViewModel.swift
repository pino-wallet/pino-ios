...
class SpeedUpAlertViewModel {
    ...
    @Published public var speedUpFeeInDollars: String = ""
    ...
    public func speedUpTransaction() {
        Web3Core.shared.speedUpTransaction(tx: transactionObject, newGasPrice: EthereumQuantity(quantity: Utilities.parseToBigUInt(newBigNumberGasPrice.description, units: .custom(0))!)).done { txHash in
            self.activityDetailsVM.performSpeedUpChanges(newTxHash: txHash, newGasPrice: self.newBigNumberGasPrice.description)
            self.coreDataManager.performSpeedUpChanges(id: self.activityDetailsVM.activityDetails.defaultActivityModel.txHash, newID: txHash, newGasPrice: self.newBigNumberGasPrice.description)
    ...
    private func calculateIncreasedGasPrice(gasPrice: EthereumQuantity) -> BigNumber {
            ...
            let bigNumberOnePercentOfGasPrice = BigNumber(number: (bigNumberGasPrice / bigNumberOneHoundered)!, decimal: 0)
            let increasedBigNumberGasPrice = BigNumber(number: bigNumberGasPrice + (bigNumberOnePercentOfGasPrice * bigNumberTen), decimal: 0)
    ...
    private func getSpeedUpDetails()  {
        Web3Core.shared.getTransactionByHash(txHash: activityDetailsVM.activityDetails.defaultActivityModel.txHash).done { txObject in
        ...
        Web3Core.shared.getGasPrice().done { gasPrice in
            ...
            let speedUpFee = self.calculateSpeedUpFee(increasedBigNumberGasPrice: increasedBigNumberGasPrice, gasUsed: txObject!.gas)
