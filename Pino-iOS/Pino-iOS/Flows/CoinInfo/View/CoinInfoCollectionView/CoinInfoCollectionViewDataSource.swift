...
extension CoinInfoCollectionView: UICollectionViewDataSource {
	...
	func recentHistoryHeaderView(kind: String, indexPath: IndexPath) -> UICollectionReusableView {
		let recentHistoryView = dequeueReusableSupplementaryView(
			ofKind: kind,
			withReuseIdentifier: ResentHistoryHeaderView.headerReuseID,
			for: indexPath
		) as! ResentHistoryHeaderView
	...
	func collectionView(
		...
		let historyCell = dequeueReusableCell(
			withReuseIdentifier: HistoryCollectionViewCell.cellID,
			for: indexPath
