	...
	private func setupBindings() {
		PendingActivitiesManager.shared.$pendingActivitiesList.sink { pendingActivities in
			...
			for pendingActivity in pendingActivities {
				let foundPendingActivityIndex = self.coinHistoryActivitiesList?
					.firstIndex(where: { $0.defaultActivityModel.txHash == pendingActivity.prev_txHash })
				if foundPendingActivityIndex != nil {
					if self.coinHistoryActivitiesList![foundPendingActivityIndex!].defaultActivityModel
						.txHash != pendingActivity.txHash {
						self
							.coinHistoryActivitiesList![foundPendingActivityIndex!] =
