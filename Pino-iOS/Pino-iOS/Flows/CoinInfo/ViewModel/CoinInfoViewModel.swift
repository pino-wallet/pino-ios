...
class CoinInfoViewModel {
    ...
    private func setupRequestTimer() {
        requestTimer = Timer.scheduledTimer(timeInterval: 12, target: self, selector: #selector(getHistoryList), userInfo: nil, repeats: true)
    }
    ...
    private func getTokenPendingActivities(pendingActivities: [ActivityModelProtocol]) -> [ActivityModelProtocol] {
        return pendingActivities.filter { pendingActivity in
            switch ActivityType(rawValue: pendingActivity.type) {
            case .swap:
                let swapPendingActivity = pendingActivity as! ActivitySwapModel
                if swapPendingActivity.detail.fromToken.tokenID == selectedAsset.id || swapPendingActivity.detail.toToken.tokenID == selectedAsset.id {
                    return true
                }
                return false
            case .transfer:
	...
	@objc private func getHistoryList() {
		...
		activityAPIClient.tokenActivities(userAddress: userAddress, tokenAddress: selectedAsset.id).sink { completed in
            ...
            if (self?.isFirstTime) != nil {
                let coinPendingActivities = self?.getTokenPendingActivities(pendingActivities: PendingActivitiesManager.shared.pendingActivitiesList)
            ...
            if self?.coinHistoryActivitiesList == nil || (self?.coinHistoryActivitiesList!.isEmpty)! {
                ...
                for activity in iteratedActivities {
                    ...
                    if self?.coinHistoryActivitiesList?[foundActivityIndex!].defaultActivityModel.failed == nil && activity
                        .failed != nil {
                        ...
                        self?.coinHistoryActivitiesList?[foundActivityIndex!] = ActivityCellViewModel(activityModel: activity)
