...
class SelectAssetToSendViewModel: SelectAssetVMProtocol {
    ...
    @Published var filteredAndSearchedAssetList: [AssetProtocol?]
    ...
    public func updateFilteredAndSearchedAssetList(searchValue: String) {
        ...
        if !searchValue.isEmpty {
            filteredAndSearchedAssetList = filteredAssetListByAmount.filter({ $0?.detail?.name.lowercased().contains(searchValueLowerCased) ?? false })
        } else {
