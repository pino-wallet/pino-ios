...
class CustomAssetViewItem: UIView {
    ...
    private func setupView() {
        ...
        if let infoIconName = infoIconName {
           let assetIconView = UIImageView(image: UIImage(named: infoIconName))
            ...
            assetIconView.pin(.fixedWidth(20), .fixedHeight(20), .relative(.trailing, -4, to: infoView, .leading), .centerY(to: superview))
        }
    ...
    private func setupConstraints() {
        ...
        tooltipIconViewButton.pin(.fixedHeight(16), .fixedWidth(16), .centerY(to: superview), .relative(.leading, 2, to: titleLabel, .trailing))
        infoView.pin(.trailing(to: superview, padding: 0), .centerY(to: superview))
    }
    
    @objc func handleOpenTooltip(_ sender: UIGestureRecognizer) {
        ...
        guard let viewController =  self.next?.next?.next?.next as? UIViewController else {
            return
        }
