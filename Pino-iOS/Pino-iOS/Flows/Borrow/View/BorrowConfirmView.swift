...
class BorrowConfirmView: UIView {
    ...
    private func setupView() {
        protocolTitleWithInfo = TitleWithInfo(actionSheetTitle: borrowConfrimVM.protocolTitle, actionSheetDescription: borrowConfrimVM.protocolActionsheetText)
        protocolTitleWithInfo.presentActionSheet = { actionSheet in
            self.presentActionSheetClosure(actionSheet)
        }
        
        feeTitleWithInfo = TitleWithInfo(actionSheetTitle: borrowConfrimVM.feeTitle, actionSheetDescription: borrowConfrimVM.feeActionSheetText)
        feeTitleWithInfo.presentActionSheet = { actionSheet in
            self.presentActionSheetClosure(actionSheet)
        }
        
        protocolInfoView = UserAccountInfoView(image: borrowConfrimVM.protocolImageName, title: borrowConfrimVM.protocolName)
    ...
    private func setupConstraints() {
        ...
        mainStackView.pin(.horizontalEdges(to: layoutMarginsGuide, padding: 0), .top(to: layoutMarginsGuide, padding: 24))
        ...
        confirmButton.pin(.horizontalEdges(to: layoutMarginsGuide, padding: 0), .bottom(to: layoutMarginsGuide, padding: 12))
    }
}
