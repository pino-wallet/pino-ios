...
class ApproveContractView: UIView {
	...
	private func setupContstraint() {
        ...
        containerView.pin(.horizontalEdges(to: layoutMarginsGuide, padding: 0), .top(to: layoutMarginsGuide, padding: 24))
        contentStackView.pin(.horizontalEdges(padding: 10), .verticalEdges(padding: 20))
        approveButton.pin(.horizontalEdges(to: layoutMarginsGuide, padding: 0), .bottom(to: layoutMarginsGuide, padding: 12))
    ...
    private func generateApproveDescriptionLabelText() -> String {
        return "\(approveContractVM.approveText) \(approveContractVM.selectedToken.symbol). \(approveContractVM.approveDescriptionText)"
    }
    
    
    @objc private func openLearnMorePage() {
        let learnMoreURL = URL(string: approveContractVM.learnMoreURL)!
        UIApplication.shared.open(learnMoreURL)
    }
    
    @objc private func onApproveButtonTap() {
        onApproveTap()
    }
}
