...
class BorrowingTokensCollectionView: UICollectionView {
    ...
    init(borrowingDetailsVM: BorrowingDetailsViewModel) {
        ...
        let flowLayout = UICollectionViewFlowLayout(scrollDirection: .horizontal, sectionInset: UIEdgeInsets(top: 0, left: 0, bottom: 0, right: 0))
    ...
    private func configureCollectionView() {
        self.borrowingDetailsProperties = borrowingDetailsVM.properties
...
extension BorrowingTokensCollectionView: UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let tokenCell = dequeueReusableCell(withReuseIdentifier: BorrowingTokenCell.cellReuseId, for: indexPath) as! BorrowingTokenCell
        
        tokenCell.borrowingTokenVM = BorrowingTokenCellViewModel(borrowinTokenModel: borrowingDetailsProperties.borrowingAssetsDetailList[indexPath.item])
