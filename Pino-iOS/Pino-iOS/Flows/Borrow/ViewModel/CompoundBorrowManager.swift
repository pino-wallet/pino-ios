    ...
    public func getBorrowData() -> TrxWithGasInfo {
        TrxWithGasInfo { seal in
            firstly {
                ...
                self.web3.getCompoundBorrowCTokenGasInfo(contractDetails: borrowContractDetails).map { (borrowContractDetails, $0) }
            }.then { borrowContractDetails, borrowGasInfo -> Promise<(GasInfo, EthereumSignedTransaction)> in
                self.web3.getCompoundBorrowTransaction(contractDetails: borrowContractDetails).map { (borrowGasInfo, $0) }
