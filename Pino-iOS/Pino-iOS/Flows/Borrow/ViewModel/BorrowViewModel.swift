...
class BorrowViewModel {
    ...
    @Published public var selectedDexProtocol: dexProtocolModel = .aave
    @Published public var userBorrowingDetails: UserBorrowingModel? = nil
...
#warning("this is mock")
    ...
    private func getUserBorrowingDetails() {
        borrowAPIClient.getUserBorrowings(address: walletManager.currentAccount.eip55Address, dex: selectedDexProtocol.type).sink { completed in
            switch completed {
            case .finished:
