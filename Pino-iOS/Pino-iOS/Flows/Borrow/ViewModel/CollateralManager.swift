...
class CollateralManager: Web3ManagerProtocol {
    ...
    private func getAaveDespositV3ERCCallData() -> Promise<String> {
        web3.getAaveDespositV3ERCCallData(assetAddress: asset.id, amount: assetAmountBigNumber.bigUInt, userAddress: walletManager.currentAccount.eip55Address)
    }
    ...
    public func getERC20CollateralData() -> TrxWithGasInfo {
        TrxWithGasInfo { seal in
            firstly {
                ...
                return self.checkAllowanceOfProvider(approvingToken: self.asset, approvingAmount: self.assetAmountBigNumber.sevenDigitFormat, spenderAddress: Web3Core.Constants.aavePoolERCContractAddress).map {
                    (signiture, $0)
                }
            }.then { signiture, allowanceData -> Promise<(String, String?)> in
                return self.getProxyPermitTransferData(signiture: signiture).map { ($0, allowanceData) }
            }.then { permitData, allowanceData -> Promise<(String, String, String?)> in
                return self.getAaveDespositV3ERCCallData().map {
