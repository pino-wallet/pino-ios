...
class CollateralIncreaseAmountViewModel {
  ...
  public var maxHoldAmount: BigNumber?
    @Published public var collateralPageStatus: CollateralPageStatus = .loading
  ...
  public var tokenImage: URL {
    ...
    public var positionErrorText: String {
        return "You have an open \(selectedToken.symbol) investment position in \(borrowVM.selectedDexSystem.name), which you need to close before depositing \(selectedToken.symbol) as collateral."
    }
...
public var newHealthScore: Double = 24
  ...
  // MARK: - Initializers
    ...
    private func checkForOpenPositionToken() -> Promise<Bool> {
        Promise<Bool> { seal in
            borrowingAPIClient.getPositionTokenId(underlyingTokenId: selectedToken.id, tokenProtocol: borrowVM.selectedDexSystem.type, positionType: .investment).sink { completed in
                switch completed {
                case .finished:
                ...
                let positionToken = self.borrowVM.globalAssetsList?.first(where: { $0.id == positionTokenID.positionID })
    ...
    private func setMaxHoldAmount() {
        self.maxHoldAmount = self.selectedToken.holdAmount
        self.collateralPageStatus = .normal
        self.destroyRequestTimer()
    }
    
    @objc private func calculateMaxHoldAmount() {
  ...
  // MARK: - Public Methods
    ...
    public func setupRequestTimer() {
        requestTimer = Timer.scheduledTimer(timeInterval: 5, target: self, selector: #selector(calculateMaxHoldAmount), userInfo: nil, repeats: true)
        requestTimer?.fire()
    }
