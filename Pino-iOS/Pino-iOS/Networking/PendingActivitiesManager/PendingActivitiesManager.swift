...
class PendingActivitiesManager {
    ...
    @Published public var pendingActivitiesList: [ActivityModelProtocol] = []
    ...
    private func getPendingActivitiesFromCoreData() {
        for activity in coreDataManager.getAllActivities() {
            ...
            case .swap:
                pendingActivitiesList.append(ActivitySwapModel(txHash: activity.txHash, type: activity.type, fromAddress: activity.fromAddress, toAddress: activity.toAddress, failed: nil, blockNumber: nil, blockTime: activity.blockTime, gasUsed: activity.gasUsed, gasPrice: activity.gasPrice))
            case .transfer:
                pendingActivitiesList.append(ActivityTransferModel(txHash: activity.txHash, type: activity.type, fromAddress: activity.fromAddress, toAddress: activity.toAddress, failed: nil, blockNumber: nil, blockTime: activity.blockTime, gasUsed: activity.gasUsed, gasPrice: activity.gasPrice))
            case .transfer_from:
                pendingActivitiesList.append(ActivityTransferModel(txHash: activity.txHash, type: activity.type, fromAddress: activity.fromAddress, toAddress: activity.toAddress, failed: nil, blockNumber: nil, blockTime: activity.blockTime, gasUsed: activity.gasUsed, gasPrice: activity.gasPrice))
            default:
    ...
    private func getActivityPendings() {
           ...
           for pendingActivity in pendingActivitiesList {
               self.requestsDispatchGroup.enter()
