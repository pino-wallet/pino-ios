...
public struct W3CompoundBorrowManager {
    ...
    public func getCAaveContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCAaveContractAddress, abi: .borrowCTokenCompound, web3: web3)
    ...
    public func getCDaiContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCDaiContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCEthContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCEthContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCUniContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCUniContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCCompContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCCompContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCLinkContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCLinkContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCUsdcContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCUsdcContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCUsdtContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCUsdtContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
    ...
    public func getCWbtcContractDetails(amount: BigUInt) -> Promise<ContractDetailsModel> {
        Promise<ContractDetailsModel> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.compoundCWbtcContractAddress, abi: .borrowCTokenCompound, web3: web3)
            let solInvolcation = contract[ABIMethodWrite.borrow.rawValue]?(amount)
            seal.fulfill(ContractDetailsModel(contract: contract, solInvocation: solInvolcation!))
        }
    }
