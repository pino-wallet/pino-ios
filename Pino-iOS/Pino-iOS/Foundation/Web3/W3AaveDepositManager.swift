...
public struct W3AaveDepositManager {
    ...
    public func getAaveDespositV3ERCCallData(assetAddress: String, amount: BigUInt, userAddress: String) -> Promise<String> {
        Promise<String> { seal in
            let contract = try Web3Core.getContractOfToken(address: Web3Core.Constants.pinoProxyAddress, abi: .aaveProxy, web3: web3)
            let solInvocation = contract[ABIMethodWrite.depositV3.rawValue]?(assetAddress.eip55Address!, amount, userAddress.eip55Address!)
