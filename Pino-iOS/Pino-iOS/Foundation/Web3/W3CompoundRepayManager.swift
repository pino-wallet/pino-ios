...
public struct W3CompoundRepayManager: Web3HelperProtocol {
    ...
    public func getRepayETHCallData(
        ...
        Promise<String> { seal in
            let contract = try! Web3Core.getContractOfToken(address: contractID, abi: .borrowCTokenCompound, web3: readWeb3)
    ...
    public func getRepayETHGasInfo(contractDetails: ContractDetailsModel) -> Promise<GasInfo> {
        Promise<GasInfo> { seal in
            gasInfoManager.calculateGasOf(method: .repayBorrow, solInvoc: contractDetails.solInvocation, contractAddress: contractDetails.contract.address!).done { gasInfo in
    ...
    public func getRepayETHTransaction(contractDetails: ContractDetailsModel, amount: BigUInt) -> Promise<EthereumSignedTransaction> {
